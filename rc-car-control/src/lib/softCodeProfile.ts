...
export function normalizeSoftCodeProfile(
  ...
  return {
    ...
    leftMotorScale: clamp(toNumber(input?.leftMotorScale, DEFAULT_SOFT_CODE_PROFILE.leftMotorScale), 0.5, 1.2),
    rightMotorScale: clamp(toNumber(input?.rightMotorScale, DEFAULT_SOFT_CODE_PROFILE.rightMotorScale), 0.5, 1.2),
    minimumMotorPower: clamp(toNumber(input?.minimumMotorPower, DEFAULT_SOFT_CODE_PROFILE.minimumMotorPower), 0, 0.45),

...
// TB6612FNG pin map. PWM uses the dedicated PWMA/PWMB inputs.
...
static const char *MOTOR_DRIVER_NAME = "TB6612";
...
void applyDrive(float throttle, float steering) {
  ...
  if (fabs(left) <= 0.02f && fabs(right) <= 0.02f) {
    ...
    setMotorRaw(PIN_BIN1, PIN_BIN2, PIN_PWMB, 0);
...
void testServosOnBoot() {
  ...
  showOledMessage(
    "SERVO TEST",
    String("CENTER P") + panDeg + " T" + tiltDeg
  );
  printServoTargets();
...
void testMotorsOnBoot() {
  if (!MOTOR_BOOT_TEST_ENABLED) return;

  Serial.println("Motor boot test: lift wheels before continuing.");
  showOledMessage("MOTOR TEST", "LEFT");
  setBootMotorPower(MOTOR_BOOT_TEST_LEFT_POWER, 0);
  delay(MOTOR_BOOT_TEST_MS);
  setBootMotorPower(0, 0);
  delay(MOTOR_BOOT_TEST_PAUSE_MS);

  Serial.println("Motor boot test: RIGHT");
  showOledMessage("MOTOR TEST", "RIGHT");
  setBootMotorPower(0, MOTOR_BOOT_TEST_RIGHT_POWER);
  delay(MOTOR_BOOT_TEST_MS);
  setBootMotorPower(0, 0);
  delay(MOTOR_BOOT_TEST_PAUSE_MS);

  Serial.println("Motor boot test: BOTH");
  showOledMessage("MOTOR TEST", "BOTH");
  setBootMotorPower(
    MOTOR_BOOT_TEST_LEFT_POWER,
    MOTOR_BOOT_TEST_RIGHT_POWER
  );
  delay(MOTOR_BOOT_TEST_MS);
  setBootMotorPower(0, 0);
  stopDrive();
  showOledMessage("MOTOR TEST", "COMPLETE");

...
The camera returns to its idle resolution about `1200 ms` after movement stops, avoiding repeated sensor reconfiguration during intermittent pan/tilt input. The realtime profile skips sensor reconfiguration entirely because its idle and motion resolutions are both QVGA. JPEG compression adapts independently inside each profile from frame size and ACK latency. Frame acknowledgements are tracked in a fixed-size ring for telemetry and tuning, but a delayed ACK never pauses capture or transmission. The camera and relay reset and increment the same frame sequence on each connection, so late acknowledgements remain measurable without adding another WebSocket message to each JPEG. The ESP32-CAM saves the selected profile in Preferences and reports FPS, frame RTT, frame size, RSSI, and timeout counters to Controller Insights. Without PSRAM it falls back to QVGA.
